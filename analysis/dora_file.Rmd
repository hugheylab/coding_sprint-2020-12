...
songLyricDT = merge(songDT, lyricDT, by = c('track_id', 'track_name'))
songLyricDT = songLyricDT[, word_danceability := ((count / word_total) * danceability)]
setorder(songLyricDT, -word_danceability)
cols = c('track_name', 'danceability', 'word_total', 'word', 'count', 'word_danceability')
head(songLyricDT[, ..cols], 20)
```
...
```
 ...
 Which genres have the most danceability
```{r}

```

```{r}
songDT[genre %like% "pop|contemporary|dance", Class := "pop"]
songDT[genre %like% "jazz|lounge|adult", Class := "jazz"]
songDT[genre %like% "rock", Class := "rock"]
songCountByClass = songDT[, .N, by = Class]
head(songCountByClass)
```


```{r}
ggplot(songDT, aes(y = danceability, x = Class)) + geom_boxplot()

```

Does danceability relate to energy?
```{r}
ggplot(songDT, aes(x = danceability, y = energy)) + geom_point() + facet_wrap(~ Class)

```

```{r}
ggplot(songDT, aes(x = danceability, y = loudness)) + geom_point() + facet_wrap(~ Class)
